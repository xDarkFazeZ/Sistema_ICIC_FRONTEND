...
import {
  ...
  CurrencyDollarIcon, CalendarDaysIcon,
...
// ─────────────────────────────────────────────────────────────────────────────
// CursosCell — muestra curso + estado de pago
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// FechasCursoCell — muestra la fechaInicio de cada curso inscrito
// ─────────────────────────────────────────────────────────────────────────────
function FechasCursoCell({ inscripciones }: { inscripciones: any[] }) {
  const [expanded, setExpanded] = useState(false);

  if (!inscripciones?.length)
    return <span className="text-xs text-default-400 italic">—</span>;
    ...
    <div className="space-y-1.5">
      {visibles.map((ins: any) => (
        <div key={ins.id} className="flex items-center gap-1.5">
          <CalendarDaysIcon className="w-3.5 h-3.5 text-default-300 shrink-0" />
          <p className="text-xs text-default-500">
            {ins.curso?.fechaInicio
              ? formatFechaCorta(ins.curso.fechaInicio)
              : "—"}
          </p>
        </div>
      ))}
    ...
    <div className="space-y-4 w-full">
      ...
      <div className="space-y-1.5">
        ...
        <Select size="sm" placeholder="Selecciona un curso" aria-label="Curso"
          ...
          {cursosFiltro.map(c => (
            <SelectItem key={String(c.id)} textValue={c.nombre}>
              <div>
...
export default function Participantes() {
  ...
  const [estadoPagoModal, setEstadoPagoModal] = useState<{
    open: boolean; inscripciones: any[]; nombreParticipante: string;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await listarCursos({ limit: 50 });
          ...
          <CardBody className="p-0">
            ...
            >
              <TableHeader>
                ...
                <TableColumn>Fecha del curso</TableColumn>
                ...
                <TableBody items={Array.from({ length: SKELETON_COUNT }, (_, i) => ({ id: i }))}>
                  {(item) => (
                    <TableRow key={`sk-${item.id}`}>
                      ...
                      <TableCell><div className="space-y-2"><Skeleton className="h-3 w-24 rounded-lg" /><Skeleton className="h-3 w-20 rounded-lg" /></div></TableCell>
                ...
                <TableBody items={paginados}>
                  {(p: any) => (
                    <TableRow key={p.id} className="hover:bg-default-50/60 transition-colors">
                      ...
                      <TableCell>
                        <FechasCursoCell inscripciones={p.inscripciones ?? []} />
                      </TableCell>
